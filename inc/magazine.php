 ...
 */
function agapanto_magazine_widgets() {
	...
	elseif ( is_page_template( 'template-magazine.php' ) && current_user_can( 'edit_theme_options' ) ) :
		...
		esc_html_e( 'Please go to Customize &#8594; Widgets and add at least one widget to the Magazine Homepage widget area.', 'agapanto' );
		echo '</p>';
...
if ( ! function_exists( 'agapanto_magazine_widget_title' ) ) :
	/**
	 ...
	 */
	function agapanto_magazine_widget_title( $widget_title, $category_id ) {
...
if ( ! function_exists( 'agapanto_magazine_entry_meta' ) ) :
	/**
	 * Displays the date and author of magazine posts
	 */
	function agapanto_magazine_entry_meta() {
...
if ( ! function_exists( 'agapanto_magazine_entry_date' ) ) :
	/**
 ...
 * @return int
 */
function agapanto_magazine_posts_excerpt_length( $length ) {
	return 15;
}
 ...
 * @return void
 */
function agapanto_flush_magazine_post_ids() {
	delete_transient( 'agapanto_magazine_post_ids' );
}
add_action( 'save_post', 'agapanto_flush_magazine_post_ids' );
add_action( 'deleted_post', 'agapanto_flush_magazine_post_ids' );
add_action( 'customize_save_after', 'agapanto_flush_magazine_post_ids' );
add_action( 'switch_theme', 'agapanto_flush_magazine_post_ids' );
