...
/**
 * Portfolio Columns Widget
 *
 * Display the latest posts from two categories in a two column layout.
 * Intended to be used in the Portfolio Page widget area to build a portfolio layouted page.
 *
 * @package Agapanto
 */
 ...
 */
class Agapanto_Portfolio_Posts_Columns_Widget extends WP_Widget
{
	...
	function __construct()
	{
		...
		// Setup Widget.
		parent::__construct(
			'agapanto-portfolio-posts-columns', // ID.
			esc_html__('Portfolio (Columns)', 'agapanto'), // Name.
			array(
				'classname'                   => 'agapanto-portfolio-columns-widget',
				'description'                 => esc_html__('Displays your posts from two selected categories.', 'agapanto'),
	...
	private function default_settings()
	{
		...
		$defaults = array(
			'category_one'       => 0,
			'category_two'       => 0,
			'category_one_title' => esc_html__('Left Category', 'agapanto'),
			'category_two_title' => esc_html__('Right Category', 'agapanto'),
			'number'             => 4,
			'highlight_post'     => true,
		);
...
?>
		...
		<div class="widget-portfolio-posts-columns widget-portfolio-posts clearfix">

			<div class="widget-portfolio-posts-content clearfix">
	...
	{
		// Get cached post ids.
		$post_ids_category_one = agapanto_get_portfolio_post_ids($this->id . '-left-category', $settings['category_one'], $settings['number']);
		$post_ids_category_two = agapanto_get_portfolio_post_ids($this->id . '-right-category', $settings['category_two'], $settings['number']);
	...
	?>
		<div class="portfolio-posts-column-left portfolio-posts-columns clearfix">

			<div class="magazine-posts-columns-content clearfix">
				<div class="portfolio-posts-column-left portfolio-posts-columns clearfix">

					<div class="portfolio-posts-columns-content clearfix">
						...
						<div class="portfolio-posts-columns-post-list clearfix">
							<?php $this->portfolio_posts($settings, $post_ids_category_one, $settings['number']); ?>
						</div>
				...
				<div class="portfolio-posts-column-right portfolio-posts-columns clearfix">
					...
					<div class="portfolio-posts-columns-content clearfix">
						...
						<div class="portfolio-posts-columns-post-list clearfix">
							<?php $this->portfolio_posts($settings, $post_ids_category_two, $settings['number']); ?>
						</div>
		 ...
		 */
		function portfolio_posts($settings, $post_ids, $number_of_posts)
		{
			if (empty($post_ids)) {
			...
			if ($posts_query->have_posts()) :
				...
				while ($posts_query->have_posts()) :
					...
					if (true === $settings['highlight_post'] && 0 === $posts_query->current_post) :
						...
						get_template_part('template-parts/widgets/portfolio-large-post', 'columns');

					else :

						get_template_part('template-parts/widgets/portfolio-small-post', 'columns');
		...
		function category_title($args, $settings, $category_id, $category_title)
		{
			...
			if (!empty($widget_title)) :
				...
				$widget_title = agapanto_portfolio_widget_title($widget_title, $category_id);
	 ...
	 * Register Widget
	 */
	function agapanto_register_portfolio_posts_columns_widget()
	{

		register_widget('Agapanto_Portfolio_Posts_Columns_widget');
	}
	add_action('widgets_init', 'agapanto_register_portfolio_posts_columns_widget');
