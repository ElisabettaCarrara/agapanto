 ...
 */
function agapanto_theme_addons_setup() {

	// Add theme support for Agapanto Pro plugin.
	add_theme_support( 'agapanto-pro' );

	// Add theme support for ThemeZee Plugins.
	add_theme_support( 'elica-breadcrumbs' );
	add_theme_support( 'elica-mega-menu', array( 'primary', 'secondary' ) );
	...
	add_theme_support(
		...
		array(
			...
			'render'         => 'agapanto_infinite_scroll_render',
		)
	);
...
add_action( 'after_setup_theme', 'agapanto_theme_addons_setup' );


/**
 * Custom render function for Infinite Scroll.
 */
function agapanto_infinite_scroll_render() {
 ...
 */
function agapanto_amp_menu_toggle() {
	if ( agapanto_is_amp() ) {
 ...
 */
function agapanto_amp_menu_is_toggled() {
	if ( agapanto_is_amp() ) {
 ...
 */
function agapanto_amp_menu_dropdown_toggles( $item_output, $item, $depth, $args ) {
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? agapanto_get_svg( 'collapse' ) : agapanto_get_svg( 'expand' );
	...
	$dropdown_button .= sprintf(
		...
		esc_attr( sprintf( "$expanded_state_id ? %s : %s", wp_json_encode( esc_html__( 'Collapse child menu', 'agapanto' ) ), wp_json_encode( esc_html__( 'Expand child menu', 'agapanto' ) ) ) ),
		esc_html( $expanded ? esc_html__( 'Collapse child menu', 'agapanto' ) : esc_html__( 'Expand child menu', 'agapanto' ) )
	);
...
add_filter( 'walker_nav_menu_start_el', 'agapanto_amp_menu_dropdown_toggles', 10, 4 );
